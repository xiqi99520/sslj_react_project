...
class drawGoodsButton extends React.Component {
	constructor(props) {
	    super(props);
	    this.state = {
	    	data:[]
			...
			success : function (data) {
				if(data.isSuccess==0){
					if(data.entity.status==2){
						window.location.href="#/order/"+that.props.match.params.userId+"/null/"+that.props.match.params.dgId+"/null/"+that.props.match.params.orderId;
					}else{
  ...
  render(){
    ...
    return (
      <div>
	  	...
	  	<p className="gd_submit to_order" onClick={this.buttonClick.bind(this)}>
	  		立即领取
	  	</p>
      </div>
    )
...
export default drawGoodsButton
